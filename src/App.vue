<template>
  <div class="h-screen bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 p-4 flex items-center overflow-hidden">
    <div class="w-full h-full flex gap-4">
      
      <!-- Left Column: The Wheel - 80% Width -->
      <div class="w-[80%] bg-white rounded-3xl shadow-2xl p-6 flex flex-col">
        ...
        <div class="flex items-center justify-between mb-4">
          ...
          <div class="flex items-center gap-3">
            ...
            <div class="flex items-center gap-1.5 px-3 py-1.5 bg-indigo-50 rounded-full">
              ...
              <span class="text-indigo-700 font-semibold text-sm">{{ names.length }}</span>
            </div>
        ...
        <!-- Canvas for Wheel - Takes full remaining space -->
        <div class="relative flex-1 flex items-center justify-center min-h-0">
          ...
          <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
            <button
              @click="spinWheel"
              :disabled="isSpinning"
              class="w-20 h-20 bg-white rounded-full shadow-2xl border-4 border-indigo-600 flex items-center justify-center transform hover:scale-110 transition-all disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
            >
              <!-- Play Icon -->
              <svg v-if="!isSpinning" class="w-10 h-10 text-indigo-600 ml-1" fill="currentColor" viewBox="0 0 24 24">
                <path d="M8 5v14l11-7z"/>
              </svg>
              <!-- Spinner Icon -->
              <svg v-else class="w-10 h-10 text-indigo-600 animate-spin" fill="none" viewBox="0 0 24 24">
          ...
          <div class="absolute -top-1 left-1/2 -translate-x-1/2">
            <div class="w-0 h-0 border-l-[24px] border-l-transparent border-r-[24px] border-r-transparent border-t-[36px] border-t-red-500 drop-shadow-lg"></div>
          </div>
        </div>
      </div>

      <!-- Right Column: Name Management - 20% Width -->
      <div class="w-[20%] bg-white rounded-3xl shadow-2xl p-4 flex flex-col min-w-[280px]">
        <h2 class="text-lg font-bold text-gray-800 mb-3 flex items-center gap-1">
        ...
        <!-- Add Name Form -->
        <form @submit.prevent="addName" class="mb-3">
          <input
            v-model="newName"
            type="text"
            placeholder="Enter name..."
            class="w-full px-3 py-2.5 text-sm border-2 border-gray-200 rounded-xl focus:border-indigo-500 focus:outline-none transition-colors mb-2"
            :disabled="isSpinning"
          />
          <button
            type="submit"
            :disabled="!newName.trim() || isSpinning"
            class="w-full py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold rounded-xl transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-1"
          >
        ...
        <div class="flex gap-2 mb-3">
          <button
            ...
            :disabled="isSpinning"
            class="flex-1 py-2 px-2 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg text-xs font-medium transition-colors disabled:opacity-50 flex items-center justify-center gap-1"
          >
          ...
          <button
            ...
            :disabled="isSpinning"
            class="flex-1 py-2 px-2 bg-red-50 hover:bg-red-100 text-red-600 rounded-lg text-xs font-medium transition-colors disabled:opacity-50 flex items-center justify-center gap-1"
          >
            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <button
          ...
          class="text-indigo-600 hover:text-indigo-800 text-xs font-medium flex items-center gap-1 mb-2"
        >
          <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <!-- Bulk Import Area -->
        <div v-if="showBulkImport" class="mb-3">
          <textarea
            v-model="bulkNames"
            rows="2"
            placeholder="John, Jane, Alex..."
            class="w-full px-3 py-2 text-xs border-2 border-gray-200 rounded-xl focus:border-indigo-500 focus:outline-none"
          ></textarea>
          <button
            @click="importBulkNames"
            class="mt-1.5 w-full py-1.5 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg text-xs font-medium transition-colors flex items-center justify-center gap-1"
          >
            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <!-- Name Tags - Scrollable -->
        <div class="flex-1 overflow-y-auto min-h-0">
          <div v-if="names.length === 0" class="text-center py-6 text-gray-400">
          ...
          <div v-else class="space-y-1.5">
            <div
              v-for="name in names"
              :key="name"
              class="group relative px-3 py-2 bg-gradient-to-r from-indigo-50 to-purple-50 text-gray-700 rounded-lg text-xs font-medium border border-indigo-100 hover:shadow-md transition-all flex items-center justify-between"
            >
              <span class="truncate">{{ name }}</span>
              <button
                @click="removeName(name)"
                :disabled="isSpinning"
                class="opacity-0 group-hover:opacity-100 transition-opacity text-red-500 hover:text-red-700 font-bold ml-1"
              >
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        >
          ...
          <div class="relative bg-white rounded-3xl shadow-2xl max-w-md w-full p-8 transform transition-all">
            ...
            <div class="text-center mt-4">
              <h3 class="text-2xl font-bold text-gray-800 mb-2">Selected Name</h3>
              
              <div class="bg-gradient-to-r from-indigo-500 to-purple-600 rounded-2xl p-6 my-6">
                <p class="text-4xl md:text-5xl font-bold text-white break-words">
                  {{ winner }}
                </p>
              </div>
              
              <p class="text-lg text-gray-600 mb-6 flex items-center justify-center gap-2">
              ...
              <div class="flex gap-3">
                <button
                  @click="spinAgain"
                  class="flex-1 py-3 px-4 bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-bold rounded-xl hover:shadow-lg transition-all flex items-center justify-center gap-2"
                >
                ...
                <button
                  @click="closeModal"
                  class="flex-1 py-3 px-4 bg-gray-200 hover:bg-gray-300 text-gray-700 font-bold rounded-xl transition-all"
                >
                  Close
                </button>
              </div>
              ...
              <button
                ...
                class="mt-3 text-sm text-red-500 hover:text-red-700 transition-colors flex items-center justify-center gap-1 mx-auto"
              >
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path>
                </svg>
        ...
        >
          ...
          <div class="relative bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6">
            <div class="text-center">
              ...
              <h3 class="text-lg font-bold text-gray-800 mb-2">{{ confirmTitle }}</h3>
              <p class="text-sm text-gray-600 mb-6">{{ confirmMessage }}</p>
              
              <div class="flex gap-3">
                <button
                  @click="handleConfirm"
                  class="flex-1 py-2.5 bg-red-600 hover:bg-red-700 text-white font-semibold rounded-lg transition-colors text-sm"
                >
                  {{ confirmButtonText }}
                </button>
                <button
                  @click="showConfirmModal = false"
                  class="flex-1 py-2.5 bg-gray-200 hover:bg-gray-300 text-gray-700 font-semibold rounded-lg transition-colors text-sm"
                >
                  Cancel
                </button>
              </div>
...
const getRandomSpinDuration = () => {
  return (4 + Math.random() * 6) * 1000 // 4-10 seconds
}
...
const determineWinner = () => {
  ...
  const angleStep = (2 * Math.PI) / names.value.length
  // Pointer is at top (-90 degrees = -PI/2)
...
const confirmClearAll = () => {
  ...
  confirmMessage.value = `Are you sure you want to delete all ${names.value.length} names from the wheel? This action cannot be undone.`
...
const shuffleDefaultNames = () => {
  names.value = getRandomDefaultNames()
  winner.value = ''
  rotation.value = rotation.value + Math.random() * 2 * Math.PI // Slight rotation change
  nextTick(() => drawWheel())
}
